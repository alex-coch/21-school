...
int main(int argc, char *argv[]) {
  ...
  if (error == FLAG_DOES_NOT_EXIST)
    fprintf(stderr,
            "usage: grep [-ivclnhso] [-e pattern] [-f file] template "
            "[file ...]\n");
  if (error == MALLOC_ERROR)
    fprintf(stderr, "grep: [malloc]: Can't allocate enough memory!\n");
  ...
  for (int i = 1; i < argc; i++) {
    if (_get_flags(argv[i], flags)) {
      ...
      if (flags->e) {
        ...
        i++;
        if ((error = _handle_flag_f(argv[i], pattern, &pattern_)) != SUCCESS) {
          break;
        }
      }
...
int _get_flags(char *argv, Flags *flags) {
  ...
  if (argv[0] == '-') {
    for (int i = 1; argv[i]; i++) {
      // if (strlen(argv) == 1 || strlen(argv) != strspn(argv, "-eivclnhsfo"))
      // error = 1;
